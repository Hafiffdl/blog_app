...
@section('title', 'Edit ' . $ki->nama)

@section('content')
    <div class="mx-auto max-w-3xl px-4 py-12 sm:px-6">
        <a href="{{ route('ki.my-submissions') }}" class="btn-outline btn-sm">
        ...
        <div class="mt-6 flex items-start gap-4">
            ...
            <div>
                <h1 class="text-2xl font-bold text-slate-900 sm:text-3xl">Edit Permohonan {{ $ki->nama }}</h1>
        ...
        <div class="card mt-8">
            <div class="card-body">
                ...
                <form action="{{ route('ki.update', $usulan->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                    @csrf
                    @method('PUT')
                            ...
                            @foreach($ki->syarat as $syarat)
                                <div>
                                    ...
                                    @elseif($syarat->tipe === 'file')
                                        <input type="file" class="form-control" id="field_{{ $syarat->id }}" name="field_{{ $syarat->id }}">
                                        <p class="mt-1.5 text-xs text-slate-400">Format: pdf, doc, docx, maks 10 MB. Kosongkan jika tidak ingin mengubah file.</p>
                                    @elseif($syarat->tipe === 'json' && $syarat->value)
                                        <select class="form-control" id="field_{{ $syarat->id }}" name="field_{{ $syarat->id }}" required>
                    ...
                    <div class="flex flex-wrap gap-3 border-t border-slate-200 pt-6">
                        ...
                        <a href="{{ route('ki.my-submissions') }}" class="btn-outline">
